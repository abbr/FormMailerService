...
exports.createUser = function(d) {
  if (users.indexOfObject('username', d.username) >= 0) {
    ...
    if (v.username == userId) {
      if (!v.superAdmin && d.username != userId) {
        ...
        require('./site').getSitesObj().forEach(function(sv) {
          if (sv.admins.indexOf(v.username) < 0) return;
          if (d.superAdmin) sv.admins.splicePositiveIndex(sv.admins.indexOf(v.username), 1);
          else sv.admins.splicePositiveIndex(sv.admins.indexOf(v.username), 1, d.username);
...
exports.deleteUser = function(userId) {
  ...
  if (userIdx == undefined) throw "Invalid username";
